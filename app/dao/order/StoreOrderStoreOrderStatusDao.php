...
class StoreOrderStoreOrderStatusDao extends BaseDao
{
     ...
     */
    public function getTakeOrderIds(array $where, int $page = 0, int $limit = 0)
    {
        return $this->search($where)->whereIn('refund_status', [0, 1])->when($page && $limit, function ($query) use ($page, $limit) {
     ...
     * @return int
     */
    public function getTakeOrderCount(array $where)
    {
        return $this->search($where)->whereIn('refund_status', [0, 1])->field([$this->alias . '.*'])->count();
    }
}
