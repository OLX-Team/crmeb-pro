...
class SystemAttachmentCategoryServices extends BaseServices
{
	...
	* @throws \FormBuilder\Exception\FormBuilderException
	 */
    public function createForm($pid, int $type = 1, int $store_id = 0)
    {
        return create_form('添加分类', $this->form(['pid' => $pid], $type, $store_id), Url::buildUrl('/file/category'), 'POST');
    }
	...
	* @throws \think\db\exception\ModelNotFoundException
	 */
    public function editForm(int $id, int $type = 1, int $store_id = 0)
    {
        $info = $this->dao->get($id);
        return create_form('编辑分类', $this->form($info, $type, $store_id), Url::buildUrl('/file/category/' . $id), 'PUT');
    }
     ...
     * @throws \FormBuilder\Exception\FormBuilderException
     */
    public function form($info = [], int $type = 1, $store_id = 0)
    {
        return [
            Form::select('pid', '上级分类', (int)($info['pid'] ?? ''))->setOptions($this->getCateList(['pid' => 0, 'type' => $type, 'store_id' => $store_id]))->filterable(true),
    ...
    {
        if ($this->dao->getOne(['name' => $data['name'], 'store_id' => $data['store_id'] ?? 0])) {
    ...
    {
        $attachment = $this->dao->getOne(['name' => $data['name'], 'store_id' => $data['store_id'] ?? 0]);
