 ...
 * @method getAllById(int $id, string $field = 'id') 通过id获取单个分类
 * @method getAllByIds(array $ids, string $field = 'id') 获取ids获取多个分类
 */
class StoreCategoryServices extends BaseServices
{
    ...
    public function form($info = [])
    {
        ...
        $f[] = Form::frameImage('pic', '分类图标(180*180)', Url::buildUrl('admin/widget.images/index', array('fodder' => 'pic')), $info['pic'] ?? '')->icon('ios-add')->width('960px')->height('505px')->modal(['footer-hide' => true]);
        $f[] = Form::frameImage('big_pic', '分类大图(468*340)', Url::buildUrl('admin/widget.images/index', array('fodder' => 'big_pic')), $info['big_pic'] ?? '')->icon('ios-add')->width('960px')->height('505px')->modal(['footer-hide' => true]);
    ...
    {
        return $this->dao->getTierList(['pid' => 0, 'is_show' => 1]);
    }
}
