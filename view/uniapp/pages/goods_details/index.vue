		...
		<view class="product-con skeleton" :style="{visibility: showSkeleton ? 'hidden' : 'visible'}">
			...
			<view>
				<scroll-view :scroll-top="scrollTop" scroll-y="true" scroll-with-animation="true"
					...
					<view id="past0">
						<productConSwiper class="skeleton-rect" :imgUrls="storeInfo.slider_image" :videoline="storeInfo.video_link"
						...
						<view class="discount acea-row row-center-wrapper" v-if="promotions_type==1">
							<view class="white acea-row row-bottom">
								<view class='timeItem acea-row row-middle skeleton-rect'>
									<countDown :is-day="false" :tip-text="'距活动结束仅剩'" :day-text="' '" :hour-text="' : '"
										:minute-text="' : '" :second-text="' '" :datatime="limitInfo.datatime"></countDown>
								</view>
								<view class="iconfont icon-jinru3" @click="seeDiscount"></view>
								<view class="left">
									<view><text class="iconfont icon-zhekoujia"></text>折扣价</view>
									<view class="money">¥
										<text class="num">{{limitInfo.discount_price.toString().split(".")[0]}}</text>
										<text v-if="limitInfo.discount_price.toString().split('.').length>1">.{{limitInfo.discount_price.toString().split(".")[1]}}</text>
									</view>
								</view>
								<view>=</view>
								<view class="y_money">
						...
						<view class="wrapper on">
							<view class="share acea-row row-between row-bottom" v-if="promotions_type !=1">
								<view class="money font-color skeleton-rect">
									...
									<image v-if="storeInfo.vip_price && storeInfo.vip_price > 0 && storeInfo.is_vip == 1 && storeInfo.price_type == 'member'"
										src="../../static/images/vip.png"></image>
									<view class="icon" v-if="storeInfo.vip_price && storeInfo.vip_price > 0 && storeInfo.is_vip == 1 && storeInfo.price_type == 'level'"><text class="iconfont icon-dengjitubiao"></text>{{storeInfo.level_name}}</view>	
								</view>
							...
							<view class="label acea-row skeleton-rect" v-if="storeInfo.is_presale_product">
								...
								<view v-text="'已预订:' + (storeInfo.fsales || 0) + (storeInfo.unit_name || '')" style="margin-left: 20rpx;"></view>
							</view>
							<view class="introduce acea-row row-between skeleton-rect">
								...
								<view class="iconfont icon-fenxiang skeleton-rect" @click="listenerActionSheet" v-if="base64Show && promotions_type ==1"></view>
							</view>
							...
							<view class="label acea-row row-between-wrapper skeleton-rect" v-if="!storeInfo.is_presale_product">
								<view v-text="'原价:￥' + (storeInfo.ot_price || 0)"></view>
						...
						<view class="wrapper">
							<view v-if="couponList.length" class="coupon acea-row row-between-wrapper skeleton-rect" @click="couponTap"
								style="margin-top: 0rpx;">
								<view class="hide line1 acea-row">
									优惠券：
									<template v-for="(item, index) in couponList">
										<view v-if="index < 2" class="activity" :key="index">
											满{{ item.use_min_price }}{{item.coupon_type==1?'减'+item.coupon_price:'打'}}<text v-if="item.coupon_type==2">{{parseFloat(item.coupon_price)/10}}折</text></view>
									</template>
								</view>
							...
							<view class="coupon acea-row row-between-wrapper" v-if="activity.length">
								<view class="line1 acea-row">
									<text>活动：</text>
									<view class="activityInfo skeleton-rect" v-for="(item, index) in activity" :key="index" @click="goActivity(item)">
							...
							<view class="coupon acea-row row-between-wrapper skeleton-rect" @click="seeDiscount" v-if="discountInfo.discount.length">
								<view class="acea-row row-middle">
									优惠：
									<view class="info line1">
										<text class="labels">{{discountInfo.discount[0].title}}</text>{{discountInfo.discount[0].desc}}
									</view>
								</view>
								<view class="iconfont icon-jiantou"></view>
							</view>
						...
						<view class="attribute skeleton-rect">
							<view v-if="attr.productAttr.length" @click="selecAttr" class="attrInfo">
							...
							<view class="specs acea-row row-between-wrapper" @click="seeSpecs" v-if="storeInfo.specs && storeInfo.specs.length">
								<view class="acea-row row-middle">
									参数：
									<view class="list line1">
										<span class="item" v-for="(item,index) in storeInfo.specs" :key="index" v-if="index<2">{{item.name}}</span>
										<span v-if="storeInfo.specs.length>2">...</span>
									</view>
								</view>
								<view class="iconfont icon-jiantou"></view>
							</view>
							<view class="ensure acea-row row-between-wrapper" @click="seeEnsure" v-if="storeInfo.ensure && storeInfo.ensure.length">
							...
							<view class="storeInfo acea-row row-between row-top" v-if="isMention && storeLen" @tap="showStoreList">
								<view class="acea-row row-top row-between">
									门店：
									<storeLis :num='1' :ids='parseInt(id)' @getStoreList="getStoreList"></storeLis>
								</view>
								<view class="iconfont icon-jiantou"></view>
							</view>
						</view>
						<discounts-list v-if="discountsData.length" :discountsData="discountsData"
							@click.native="goDiscounts">
						</discounts-list>
					</view>
					<navigator hover-class="none" :url="'/pages/users/goods_comment_list/index?product_id=' + id" class="userEvaluation skeleton-rect" id="past1" v-if="replyCount">
					...
					<!-- 优品推荐 -->
					<view class="superior skeleton-rect" v-if="good_list.length" id="past2">
						...
						<view class="slider-banner banner">
							<swiper :indicator-dots="good_list.length==1?false:true" :autoplay="autoplay" :circular="circular" :interval="interval"
								...
								<swiper-item v-for="(item, indexw) in good_list" :key="indexw" v-if="indexw<2">
					...
					<view class="product-intro skeleton-rect" id="past3">
			...
			<view class="footer acea-row row-between-wrapper">
				...
				<navigator open-type="reLaunch" class="animated item skeleton-rect" :class="animated == true ? 'bounceIn' : ''"
				...
				<view class="bnt acea-row skeleton-rect" :class="!storeInfo.cart_button?'fictitious' : ''" v-if="attr.productSelect.stock <= 0">
					<form v-if="storeInfo.cart_button" @submit="joinCart" class="joinCart bnts"><button class="joinCart bnts"
				...
				<view class="bnt acea-row skeleton-rect" :class="!storeInfo.cart_button?'fictitious' : ''" v-else>
					<form v-if="storeInfo.cart_button" @submit="joinCart" class="joinCart bnts"><button class="joinCart bnts"
							form-type="submit">加入购物车</button></form>
					<form v-if="storeInfo.is_presale_product&&(storeInfo.presale_pay_status == 1 || storeInfo.presale_pay_status == 3)" class="buy bnts bg-color-hui"><button class="buy bnts bg-color-hui"
							form-type="submit">{{storeInfo.presale_pay_status === 1?'未开始':'已结束'}}</button></form>	
					<form v-else @submit="goBuy" class="buy bnts"><button class="buy bnts" form-type="submit">立即购买</button></form>
				</view>
			</view>
			<shareRedPackets :sharePacket="sharePacket" :showAnimate="showAnimate" @listenerActionSheet="listenerActionSheet" @boxStatus="boxStatus"
				@closeChange="closeChange" v-if="base64Show"></shareRedPackets>
			<!-- 组件 -->
			...
			<view class="generate-posters" :class="posters ? 'on' : ''">
				<view class="generateCon acea-row row-middle">
					...
					<!-- #endif -->
					<!-- #ifdef MP -->
					<button class="item" open-type="share" hover-class="none" id="share2">
						<!-- <view class="iconfont icon-weixin3"></view> -->
						<view class="pictrue">
							<image src="../../static/images/weixin.png"></image>
						</view>
				...
				uni.getImageInfo({
					...
					success() {
						if (arr2[2] == '') {
							...
							//生成推广海报
							that.$nextTick(function(){
								that.$util.PosterCanvas(that.fontColor, that.themeColor, that.siteName, arr2, that
