	...
	<view :style="colorStyle">
		<view class='shoppingCart copy-data'>
			...
			<view class='nav acea-row row-between-wrapper'>
				<view><text class='num font-num'>{{cartCount}}</text>购物数量</view>
									...
									<navigator :url='"/pages/goods_details/index?id="+item.product_id' hover-class='none'
										...
										<view class='carnum acea-row row-center-wrapper' v-if="item.attrStatus && !item.is_gift">
											...
											<view class="plus" :class="item.numAdd ? 'on' : ''"
												@click.stop='addCart(jindex,index)'>+</view>
										</view>
									</navigator>
		...
		<!-- 		<view class='loadingicon acea-row row-center-wrapper' v-if="cartList.valid.length&&!cartList.invalid.length">
			...
			<view class="tips acea-row row-middle" v-if="isTips"><text class="iconfont icon-tishi"></text>部分活动不能叠加，系统已自动为您计算最优惠的价格</view>
			<view class='footer acea-row row-between-wrapper' :class="newData.status && newData.status.status?'':'on'"
				...
				<view class='money acea-row row-middle' v-if="footerswitch==true">
					<view class="left">
						实付：
						<text class='font-color'>￥{{$util.$h.Sub(selectCountPrice,discountInfo.deduction.coupon_price)<=0?0:$util.$h.Sub(selectCountPrice,discountInfo.deduction.coupon_price)}}</text>
						<view class="detailed" @click="discountTap" v-if="Object.prototype.toString.call(discountInfo.coupon) === '[object Object]'">优惠明细<text class="iconfont" :class="discountInfo.discount?'icon-xiangxia':'icon-xiangshang'"></text></view>
					</view>
		...
		<view class="foot" v-if="newData.status && newData.status.status">
			<view class="page-footer" id="target" :style="{'background-color':newData.bgColor.color[0].item}">
				<view class="foot-item" v-for="(item,index) in newData.menuList" :key="index" @click="goRouter(item)">
					...
					<div class="count-num"
						v-if="item.link === '/pages/order_addcart/order_addcart' && cartCount > 0">
						{{cartCount}}
					</div>
				</view>
			</view>
...
<script>
	...
	export default {
		...
		methods: {
			...
			goCollect(item){
				uni.navigateTo({
					url: `/pages/goods_list/index?sid=0&title=默认&promotions_type=${item.promotions_type}&promotions_id=${item.id}`
				})
			},
			myDiscount(){
				...
				if (selectValue.length > 0) {
					...
					if(Object.prototype.toString.call(coupon) === '[object Object]' && !coupon.used){
						setCouponReceive(this.discountInfo.coupon.id).then(res=>{
							uni.navigateTo({
								url: '/pages/users/order_confirm/index?cartId=' + selectValue.join(',') + '&couponId=' + res.data.id +'&couponTitle='+ coupon.coupon_title
							});
						}).catch(err=>{
							return that.$util.Tips({
								title: err
						...
						if(Object.prototype.toString.call(coupon) === '[object Array]'){
							...
							url = '/pages/users/order_confirm/index?cartId=' + selectValue.join(',') + '&couponId=' + coupon.used.id +'&couponTitle='+ coupon.coupon_title
						}
						uni.navigateTo({
							url: url
						});
			...
			addCart: Debounce(function(jindex,index) {
				let that = this;
				let item = that.cartList.valid[jindex].valid[index];
				item.cart_num = Number(item.cart_num) + 1;
			...
			manage: function() {
				...
				// let newValid = that.cartList.valid.map(j => {
				// 	j.valid.map(item=>{
