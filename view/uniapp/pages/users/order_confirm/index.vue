<template>
	<view  :style="colorStyle">
		<view class='order-submission'>
			<view class="allAddress" :style="store_self_mention ? '':'padding-top:10rpx'" v-if="product_type == 0">
			...
			<view class='footer acea-row row-between-wrapper'>
				...
				<view class='settlement' style='z-index:100' @tap.stop="goPay" v-if="(((valid_count>0&&!discount_id) || (valid_count==cartInfo.length&&discount_id)) && shippingType) || (!shippingType && addressId)">立即结算</view>
		...
		methods: {
			...
			showStoreList: function() {
				...
				if (this.storeList.length > 0) {
					uni.navigateTo({
						url: `/pages/users/goods_details_store/index?type=1&cartId=${_this.cartId}&new=${_this.news}&pinkId=${_this.pinkId}&couponId=${_this.couponId}`
					})
				}
			},
			changePayType(type) {
